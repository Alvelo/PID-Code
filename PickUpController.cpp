...
{
   // le resta el error viejo con el error nuevo  que esta tienendo el robot actualmente.
    double error = error - blockYawError;
    // suma el error que se le estaba restando, con el error que esta teniendo actualmente para tener un margen de error total.
    double allerror;
    // el error actual que esta teniedo se esta guardando en la variable P.
    double P =  blockYawError;
    // aqui se hara la derivada, y se guardara en la variable D.
    double D;
    // se hara el integral y se guardara en la variable I.
    double I;
    // estos son declarados como constants gains.
    double Ki = 0.10;
    double Kg;
    double Kp = 0.20;
    double Kd = 0.15;
  ...
  if (!targetHeld)
  {
    ...
    {  
      ...
      vel = 0.09;
       ...
       allerror = error + P;
        // Derivada
        D =  (allerror - P)/current_time;
        // Integral      
        I = allerror/current_time;
        //constant gains
        Ki = Ki + I;
        // la suma y multiplicacion de todos los constant gains.
        Kg = P*Kp + I*Ki + D*Kd;
        
        result.pd.cmdVel = vel;
        result.pd.cmdAngularError = Kg;

      
      
      
      cout << "cmdVel: " << result.pd.cmdVel << endl;
      // hace la curva hasta que sintralize... = blockYawError.
      result.pd.cmdAngularError = -blockYawError;
      //Delete statement
      timeOut = false;
